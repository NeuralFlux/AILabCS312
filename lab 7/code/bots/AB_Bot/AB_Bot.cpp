...
*/
int minimax( OthelloBoard board, int depth, bool maximisingPlayer, Turn turn, int alpha, int beta );
...
int minimax( OthelloBoard board, int depth, bool maximisingPlayer, Turn turn, int alpha, int beta ) {
    ...
    if(maximisingPlayer) {
        ...
        for(it = moves.begin(); it != moves.end(); ++it) {
            ...
            // Get its Eval
            int eval = minimax( boardCopy, depth - 1, false, other(turn), alpha, beta );
        ...
        for(it = moves.begin(); it != moves.end(); ++it) {
            ...
            // Get its Eval
            int eval = minimax( boardCopy, depth - 1, true, other(turn), alpha, beta );

            // Update best eval and Beta
    ...
    */
    for(int depth = 3; depth < max_depth; ++depth) {
        ...
        for(list<Move>::const_iterator move=moves.begin(); move != moves.end(); ++move) {
            ...
            // Get its eval
            eval = minimax(boardCopy, depth, false, other(turn), NEG_INF, POS_INF);
