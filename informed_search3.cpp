...
void reset1(vector<vector<Node>>& graph) {
...
float distance(pair<int,int> one,pair<int,int> two) {
    int a = (one.first - two.first)*(one.first - two.first);
    int b = (one.second - two.second)*(one.second - two.second);
    float dis = sqrt(a+b);
    return dis;
}

void MoveGen1(Node &node, int max_depth, vector<pair<int,int>>& Neighbors,vector<vector<Node>>& graph) {
        ...
        for(int i = 0; i < 4; i++) {
            if(neighbors[i].first != -1 && neighbors[i].second != -1 && node.depth + 1 <= max_depth && graph[neighbors[i].first][neighbors[i].second].found != true) {
                ...
                char curr_char = graph[neighbors[i].first][neighbors[i].second].data;
                ...
                reset1(graph);
                if(max_depth-1 >= 0){
                    MoveGen1(graph[neighbors[i].first][neighbors[i].second], max_depth-1, Neighbors, graph);
                }
...
public:
    ...
    */
    ...
    class Queue {
    private:
        pair<int,int> items[tabu_tenure];
        int front, rear;
    ...
    public:
        Queue(){
            ...
            for(int i=0; i<tabu_tenure; i++){
                this->items[i].first = 0;
                this->items[i].second = 0;
            }
        }
        ...
        pair<int,int> item(int k) {
            pair<int,int> value;
            value.first = items[k].first;
            value.second = items[k].second;
            return value;
        }
        ...
        void enQueue(pair<int,int> element){
            if(isFull()){
                //cout << "Queue is full";
            } else {
                if(front == -1) front = 0;
                rear = (rear + 1) % tabu_tenure;
                items[rear].first = element.first;
                items[rear].second = element.second;
            }
        }
        ...
        pair<int,int> deQueue(){
            ...
            if(isEmpty()){
               // cout << "Queue is empty" << endl;
                ...
                element.first = items[front].first;
                element.second = items[front].second;
                if(front == rear){
                    front = -1;
                    rear = -1;
                } 
        ...
        while(!open.empty())    {
            if(goalTest(open.top()->coord, dest_coord)) {
                        ...
                        //if(heuristic(current_coord) > heuristic(neighbors[i])){
                            for(int k=closed.fronts(); k!=closed.rears(); k=(k+1)%tabu_tenure){
                                if(closed.item(k).first != neighbors[i].first && closed.item(k).second != neighbors[i].second) {
                                    open.push(&graph[neighbors[i].first][neighbors[i].second]);
                                    if(heuristic(best) < heuristic(neighbors[i])){
                ...
                if(open.size() == 0){
                    for(int k=closed.fronts(); k!=closed.rears(); k=(k+1)%tabu_tenure) {
                        for(int i = 0; i < 4; i++) {
                            if(closed.item(k).first == neighbors[i].first && closed.item(k).second == neighbors[i].second && heuristic(neighbors[i]) < heuristic(best)) {
                                open.push(&graph[neighbors[i].first][neighbors[i].second]);
                                best.first = neighbors[i].first;
                                best.second = neighbors[i].second;
                            }
